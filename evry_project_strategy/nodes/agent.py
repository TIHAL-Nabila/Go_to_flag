...
def run_demo():
    ...
    while not rospy.is_shutdown():
        ...
        if (robot.robot_name== 'robot_2'):
             distance = float(robot.getDistanceToFlag())
             print(f"{robot_name} distance to flag = ", distance)
            # rospy.sleep(1.5)
             err2=distance-d_des
             vt=Kp*err2+Ki*(err_old_02+err2)+Kd*(err2-err_old_02)/dt
             robot.set_speed_angle(vt, angle)
             err_old_02=err2
             
        if (robot.robot_name== 'robot_3'):
             distance = float(robot.getDistanceToFlag())
             print(f"{robot_name} distance to flag = ", distance)
             #rospy.sleep(3.0)
             err3=distance-d_des
             vt=Kp*err3+Ki*(err_old_03+err3)+Kd*(err3-err_old_03)/dt
             robot.set_speed_angle(vt, angle)
             err_old_03=err3
